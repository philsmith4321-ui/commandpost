...
The fifth check proves nginx is the ONLY path to the app. The four https checks all go through nginx and would pass even if the raw app port sat open to the internet — CommandPost has no in-app auth, so a reachable `:3004` serves every gated surface with zero credentials. The repo's `ecosystem.config.js` starts Next with `-H 127.0.0.1` (loopback-only bind). PM2 caches script+args in its dump, so an ecosystem change only takes effect via `pm2 delete commandpost && pm2 start ecosystem.config.js && pm2 save` on the droplet. Do **not** use `pm2 restart` (keeps old args) or `pm2 startOrReload` (observed 2026-07-17: it grafted the new args onto the previously-cached script and crash-looped the app). If the fifth check returns anything but a connection failure, stop here. *(Applied and verified in prod 2026-07-17: gates 401, `:3004` connection-refused.)*
